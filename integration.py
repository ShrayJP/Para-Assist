...
while True:
    ...
    if landmark_points:
        ...
        try:
            active_window = pyautogui.getWindowsWithTitle('Active Window')[0]
            active_window_rect = active_window.geometry
            mouse_pos = pyautogui.position()
            if active_window_rect.contains(mouse_pos):
                print("Mouse is over an active window")
                # Start speech recognition
                real_time_recognition()
            else:
                print("Mouse is not over an active window")
                # Stop speech recognition
                r.stop_listening = True
        except IndexError:
            pass
